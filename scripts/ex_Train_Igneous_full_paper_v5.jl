...
function TrainIgneous(;
    ...
    # Switches
    create_data_fig = true

    # Create the data
    if create_data
        T_range  = (652.0, 1298.0)
        P_range  = (0.12,     9.98)
        Si_range = (45.0,    71.0)
        ini_lev  = 5
        sub_lev  = 2
    ...
    # Reorder outputs for easy access in the heads
    ...
    dev     = cpu_device()
    dev_str = "CPU"
    size(input_shuff, 2) > 100_000 ? (dev = gpu_device(); dev_str = "GPU") : nothing
    ...
    # Train the model
    @printf "Running bsize = %.5g \t HD = %5g \t HL = %.5g \t epochs = %5g \n" batchsize hidden_dims hidden_lays epochs
    hyperparams = Dict("bsize" => batchsize, "θ1"=> 0.0, "θ2" => 1.0) |> dev
    _ = MAGEMin_MLPs.train_model(input_shuff, output_ord, dev, rng; nEpochs = epochs, hidden_dims = hidden_dims, hidden_lays = hidden_lays, hyperparams = hyperparams, nprint = 25, save_model = true, fname_root = "./user/TrainedModel_$(hidden_dims)HD_$(hidden_lays)HL_$(batchsize)batchsize_$(epochs)epochs", log_transform, DatType = DatType)
